...
static void debug_tap_connect_request(protocol_binary_request_header *req)
{
    ...
    size_t nextbyte = sizeof(*req);
    protocol_binary_request_tap_connect *t = (void*)req;
    printf("Flags        (%02zu-%02zu): ",
    ...
    if (keylen > 0) {
        printf("Name         (%02zu-%02zu): [", nextbyte, nextbyte + keylen - 1);
    ...
    if (flags & TAP_CONNECT_FLAG_BACKFILL) {
        ...
        printf("Backfill date(%02zu-%02zu): ",
    ...
    if (flags & TAP_CONNECT_FLAG_LIST_VBUCKETS) {
        ...
        num = ntohs(val);
        printf("VBucket list (%02zu-%02zu): \n",
               nextbyte, nextbyte + (2U * num) + 1);

        printf("    # listed (%02zu-%02zu): ",
...
static void debug_tap_common_body(protocol_binary_request_tap_no_extras *req) {
    size_t nextbyte = sizeof(req->message.header);
    printf("Engine priv. (%02zu-%02zu): ",
           nextbyte,
...
static void debug_tap_mutation_request(protocol_binary_request_header *req)
{
    ...
    size_t nextbyte = sizeof(protocol_binary_request_tap_no_extras);
    printf("Item Flags   (%02zu-%02zu): ",
    ...
    uint16_t keylen = ntohs(req->request.keylen);
    if (keylen > 0) {
        printf("Key          (%02zu-%02zu): [", nextbyte, nextbyte + keylen - 1);
    ...
    if (bodylen > 0) {
        printf("Value        (%02zu-%02zu): [", nextbyte, nextbyte + keylen - 1);
...
{
    size_t nextbyte = sizeof(*res);
    uint32_t length = ntohl(res->response.bodylen);
    if (res->response.status == 0) {
        printf("Mechanisms   (%02zu-%02zu): ",
               nextbyte, nextbyte + length - 1);
    } else {
        printf("Error message(%02zu-%02zu): ",
               nextbyte, nextbyte + length - 1);
    }
